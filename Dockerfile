...
ADD https://ftp.gnu.org/gnu/gcc/gcc-14.1.0/gcc-14.1.0.tar.gz /root/src/
ENV TARGET=i686-elf \
...
RUN apt-get update && apt-get -y upgrade; \
    ...
    ../binutils-2.42/configure --target=$TARGET --prefix="$PREFIX" --disable-multilib --disable-nls --disable-werror; \
    make -j$(nproc) && make install; \
    cd $HOME/src && mkdir build-gcc && cd build-gcc; \
    ../gcc-14.1.0/configure --target=$TARGET --prefix="$PREFIX" --disable-multilib --disable-nls --enable-languages=c,c++ --without-headers; \
